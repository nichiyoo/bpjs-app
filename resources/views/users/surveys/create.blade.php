<x-app-layout>
    ...
    <div class="p-8 my-12 space-y-8 bg-white border border-neutral-200 rounded-xl">
        <x-header as="h3">
            <x-slot name="title">
                {{ __('Tambah Data Survei') }}
            </x-slot>

            <x-slot name="description">
                {{ __('Tambahkan data survei, dan pastikan data yang dikirim benar') }}
            </x-slot>
        </x-header>

        <form action="{{ route('users.surveys.store') }}" method="post" class="grid gap-6 lg:grid-cols-2">
            @csrf
            ...
            <div>
                ...
                <x-select id="duration" name="duration" value="{{ old('duration') }}" required>
                    @foreach ($durations as $item)
                        <option value="{{ $item }}" @if ($item == old('duration')) selected @endif>
                            {{ $item }}
                        </option>
                    @endforeach
                </x-select>
            ...
            <div class="col-span-full">
                ...
                <x-input id="sample" type="number" name="sample" value="{{ old('sample') }}" required />
                <x-error :value="$errors->get('sample')" />
            </div>

            <div class="col-span-full">
                <x-label for="status" :value="__('survey.status.label')" />
                <x-select id="status" name="status" value="{{ old('status') }}" required>
            ...
            <div class="col-span-full">
                ...
                <x-textarea id="annotation" name="annotation" value="{{ old('annotation') }}" rows="3" />
                <x-error :value="$errors->get('annotation')" />
            </div>

            <div>
                <x-label for="document" :value="__('survey.document.label')" />
                <x-input id="document" type="number" name="document" value="{{ old('document') }}" required />
                <x-error :value="$errors->get('document')" />
            </div>

            <div>
                <x-label for="entry" :value="__('survey.entry.label')" />
                <x-input id="entry" type="number" name="entry" value="{{ old('entry') }}" required />
